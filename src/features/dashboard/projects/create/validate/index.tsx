...
const baseSchema = z.object({
  type: z.enum(["チラシ作成", "LP修正依頼", "動画編集", "名刺作成", "バナー作成", "Instagram投稿", "SEO記事作成", "サムネイル作成", "LINE構築", "台本作成"]),
...
const flyerSchema = baseSchema.extend({
...
const lpModificationSchema = baseSchema.extend({
  ...
  // 4. 修正の目的と意図
  modificationPurpose: z.array(z.enum([
    "コンバージョン率改善（CVR）",
    "情報の最新化（価格・期間など）",
    "デザインの印象を変えたい",
    "スマホ対応強化／UI改善",
...
const videoEditingSchema = baseSchema.extend({
  ...
  // 2. 素材提供について
  sourceVideoUrl: z.string().min(1, "素材動画のURL又はアップロード情報は必須です"),
  additionalImages: z.array(z.string()).optional(),
  bgmRequirements: z.string().optional(),

  // 3. 編集内容の指示
  editingStyle: z.array(z.enum([
    "ポップ",
    "シネマティック",
    "ビジネス寄り",
    "テロップ多め",
...
const bannerSchema = baseSchema.extend({
  ...
  // 4. デザインイメージ
  designAtmosphere: z.array(z.enum([
    "高級感",
    "ポップ",
    "シンプル",
    "かわいい",
...
const scriptSchema = baseSchema.extend({
  ...
  tonePreferences: z.array(z.enum([
    "砕けた・フレンドリー",
    "信頼感・誠実系",
